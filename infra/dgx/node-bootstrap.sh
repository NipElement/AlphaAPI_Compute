...
# --- 5. storage sentinel (GPU nodes) ---------------------------------------
if [[ "$ROLE" == gpu ]]; then
  say "NVMe RAID sentinel"
...
say "done ($ROLE). Next: kubeadm $( [[ $ROLE == head ]] && echo 'init --config kubeadm-cluster-config.yaml' || echo 'join <printed by the head node>')"
