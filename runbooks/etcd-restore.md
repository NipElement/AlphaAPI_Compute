...
> ⚠ **快照里有 Secret,所以离机的每一份拷贝都是凭据**。2026-09-01 起 Secret 在 etcd 里
> 是**静态加密**的(`encryption-provider-config`,钥匙在 `/etc/kubernetes/enc/keys.yaml`,
> 0600,不进 Git、不进快照)。所以一份被偷走的快照**不再等于**拿到台账的 HMAC 钥匙 ——
> 这正是它此前的样子:`strings etcd-*.db` 一条命令。**加密钥匙不要和快照放在一起**,
> 否则等于没加密。对头节点的 root 这一切都不成立(他能同时读钥匙、台账和 Prometheus);
> 这道防线守的是「离开了机房的那份拷贝」。
