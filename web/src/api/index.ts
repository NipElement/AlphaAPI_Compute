...
export const auth = {
  ...
  changePassword: (current: string, next: string) =>
    api.post<{ ok: boolean; note?: string }>("/auth/password", {
      current,
      new: next,
    }),
  ...
  deleteUser: (name: string) =>
    api.del<{ deleted: string }>(`/auth/users/${encodeURIComponent(name)}`),
};
...
export const papi = {
  ...
  instances: (ns: string, workload: string) =>
    api.get<{ instances: Instance[] }>(
      withNs(`/papi/instances?workload=${encodeURIComponent(workload)}`, ns),
    ),
  logs: (ns: string, pod: string, tail = 300) =>
    api.get<{ pod: string; log: string }>(
      withNs(`/papi/logs?pod=${encodeURIComponent(pod)}&tail=${tail}`, ns),
    ),
  events: (ns: string, name: string) =>
    api.get<{ events: EventRow[] }>(
      withNs(`/papi/events?name=${encodeURIComponent(name)}`, ns),
    ),
  createDevmachine: (ns: string, body: unknown) =>
    api.post(withNs("/papi/devmachines", ns), body),
  rotateDevmachineKey: (ns: string, name: string, sshPublicKey: string) =>
    api.put(
      withNs(`/papi/devmachines/${encodeURIComponent(name)}/ssh-key`, ns),
      { sshPublicKey },
    ),
  createJob: (ns: string, body: unknown) =>
    api.post(withNs("/papi/jobs", ns), body),
  createService: (ns: string, body: unknown) =>
    api.post(withNs("/papi/services", ns), body),
  createVolume: (ns: string, body: unknown) =>
    api.post(withNs("/papi/volumes", ns), body),
