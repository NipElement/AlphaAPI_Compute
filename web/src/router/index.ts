...
router.beforeEach(async (to) => {
  ...
  if (authStore.loadError) return false;
...
// A stale tab may request a removed content-hashed chunk after a deployment.
// Keep a visible recovery action available from the initial bundle; do not
// leave the old page under a new URL or loop automatic reloads.
export const navigationFailure = ref("");
router.onError((error, to) => {
  navigationFailure.value =
    error instanceof Error ? error.message : String(error);
  if (to) window.history.replaceState(null, "", router.resolve(to).href);
});
